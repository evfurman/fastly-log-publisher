...
const logGroupName = process.env.LogGroupName //Name of the log group goes here;
const logStreamName = process.env.LogStreamName //Name of the log stream goes here;
...
exports.handler = (event, context, callback) => {
    ...
    function putLogEvents (sequenceToken, logData) {
        ...
        rl.on('line', (line) => {
            ...
            var tsDirty = line.split(' ', 1)[0];
            var re = /<[^>]*>/;
            var ts = tsDirty.replace(re, '');
            ...
            var slicedLine = line.split(' ').slice(3).join(' ');
